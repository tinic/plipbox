...
typedef u08  (*pio_dev_mcast_filter_t)(const u08 hash[8]);
...
static inline u08 pio_dev_mcast_filter(pio_dev_ptr_t pd, const u08 hash[8])
{
  pio_dev_mcast_filter_t filter_f =
    (pio_dev_mcast_filter_t)pgm_read_word(&pd->mcast_filter_f);
  return filter_f ? filter_f(hash) : PIO_NOT_FOUND;
}
